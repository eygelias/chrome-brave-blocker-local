...
v3.0.1 aplica cierre seguro: descarta antes de convertir reglas con modificadores no representables, incluidos los modificadores conocidos que el conversor ignora silenciosamente, como `$app` y `$referrerpolicy`. Una segunda guarda interpreta anclas y comodines DNR contra URL HTTP/HTTPS diversas, rechazando patrones que cubren un esquema completo como `|h*`, filtros universales formados solo por anclas (`|*`, `*|`, `|*|` y equivalentes) y cualquier regex sin alcance de dominio. `scripts/test_converter_safety.mjs` reproduce estos patrones, incluso con entradas CRLF de Windows, y se ejecuta mediante `npm test`. Los totales descartados quedan registrados en cada `build-info.json`.
