...
v3.0.1 fails closed: rules containing unrepresentable modifiers are discarded before conversion. This explicitly includes known silently ignored modifiers such as `$app` and `$referrerpolicy`. A second structural guard interprets DNR anchors and wildcards against diverse HTTP, HTTPS, WS, and WSS URLs, rejecting scheme-wide patterns such as `|h*` or `|w*`, anchor-only universal filters (`|*`, `*|`, `|*|`, and equivalents), and every unscoped regex block. `scripts/test_converter_safety.mjs` reproduces these failure patterns, including Windows CRLF inputs, and runs through `npm test`. Sanitization and pruning totals are recorded in each `build-info.json`.
...
v3.0.1 aplica cierre seguro: descarta antes de convertir reglas con modificadores no representables, incluidos los modificadores conocidos que el conversor ignora silenciosamente, como `$app` y `$referrerpolicy`. Una segunda guarda interpreta anclas y comodines DNR contra URL HTTP, HTTPS, WS y WSS diversas, rechazando patrones que cubren un esquema completo como `|h*` o `|w*`, filtros universales formados solo por anclas (`|*`, `*|`, `|*|` y equivalentes) y cualquier regex sin alcance de dominio. `scripts/test_converter_safety.mjs` reproduce estos patrones, incluso con entradas CRLF de Windows, y se ejecuta mediante `npm test`. Los totales descartados quedan registrados en cada `build-info.json`.
